<<<<<<< HEAD:Vision_Detection/face_detection.py
...
class FaceMeshDetector:
    ...
    #Parameters object for mash and picture frame that was caught
    def process(self, frame_bgr) -> Optional[List[Tuple[int, int]]]:
        h, w = frame_bgr.shape[:2]          #Taking the dimension of caught picture
        frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)       #Change of picture color space because mediapipe works with RBG and openCV with RGB
        ...
        #Take the first detected face (ofc this is goining to be our only face)
        #Every cordinate is turnd in pixel (we have 468 of them)
        landmarks = result.multi_face_landmarks[0].landmark     #Main problem is that mediapipe returns relative distants and not pixels (number between 0 and 1)
        coords = [(int(lm.x * w), int(lm.y * h)) for lm in landmarks]
        return coords

    #Finding smalles and biggest cordinate in order to make a box where face is
    @staticmethod
    def compute_bbox(coords: List[Tuple[int, int]]) -> Tuple[int, int, int, int]:
        xs = [x for x, _ in coords]     #Taking just x cordinates from landmarks
        ys = [y for _, y in coords]     #Taking just x cordinates from landmarks
=======
from typing import List, Tuple, Optional    #Importing new type for easier function returns
import cv2
import mediapipe as mp
...
>>>>>>> lorenzobranch:01_Detect_Crop_Align/vision_detection/face_detection.py
